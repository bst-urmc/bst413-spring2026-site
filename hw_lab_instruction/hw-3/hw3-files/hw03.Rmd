...
Likelihood: $p(y|\alpha,\beta) = \prod_{j=1}^4\theta^{y_i}(1-\theta_j)^{n_j-y_j}$, where $\theta_j = (1+exp(-\alpha-\beta*d_j))^{-1}$ and $d_j$ is the $j^{th}$ dose.
...
`r a*100`% of my samples had $\beta>0$.

In this problem, $\beta>0$ means that there is an increasing relationship between dose and death.
...
For the code in (a), the log posterior would now include parts for the two normal priors, but these are just dnorm(alpha,0,2, log=TRUE) and dnorm(beta,0,10, log=TRUE) and get added to the likelihood part in the code.

For the code in (c), the priors just gets changed for $\alpha$ and $\beta$ in the model section:
...
&\propto& \prod_{j=1}^{10} \left[p(y_j|n_j,\theta_j)p(\theta_j|\alpha,\beta)\right]
p(\alpha, \beta) \\
%&\propto& \prod_{j=1}^{10} \left[Binomial(y_j|n_j,\theta_j)Beta(\theta_j|\alpha,\beta)\right]
%\frac{\alpha\beta}{(\alpha+\beta)^{-5/2}} \\  %WRONG
&\propto& \prod_{j=1}^{10} \left[Binomial(y_j|n_j,\theta_j)Beta(\theta_j|\alpha,\beta)\right]
(\alpha+\beta)^{-5/2} \\
&=& \prod_{j=1}^{10} \left[\left({n_j\choose y_j} \theta_j^{y_j}(1-\theta_j)^{n_j-y_j}\right)
\left(\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}\theta_j^{\alpha-1}(1-\theta_j)^{\beta-1}\right)\right]
(\alpha+\beta)^{-5/2} \\
&\propto& \prod_{j=1}^{10} \left[\theta_j^{\alpha+y_j-1}(1-\theta_j)^{n_j-y_j+\beta-1}\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}\right]
(\alpha+\beta)^{-5/2} \\
&\propto& \prod_{j=1}^{10} \left[\theta_j^{\alpha+y_j-1}(1-\theta_j)^{n_j-y_j+\beta-1}\right] 
(\alpha+\beta)^{-5/2}\left(\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}\right)^{10}
\end{eqnarray}

Dropping everything that is not $\theta$:
\begin{eqnarray*}
p(\theta \mid \alpha, \beta, y) &\propto& \prod_{j=1}^{10} \left[\theta_j^{\alpha+y_j-1}(1-\theta_j)^{n_j-y_j+\beta-1}\right] \\
&\propto& \prod_{j=1}^{10} Beta(\theta_j|\alpha+y_j,\beta+n_j-y_j)
\end{eqnarray*}

Integrating ten times over all the $\theta$'s:
\begin{eqnarray*}
p(\alpha, \beta \mid y) 
&\propto& \int \prod_{j=1}^{10} \left[\theta_j^{\alpha+y_j-1}(1-\theta_j)^{n_j-y_j+\beta-1}\right] 
(\alpha+\beta)^{-5/2}\left(\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}\right)^{10} d\theta \\
&=& (\alpha+\beta)^{-5/2}\left(\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}\right)^{10}
\int \prod_{j=1}^{10} \left[\theta_j^{\alpha+y_j-1}(1-\theta_j)^{n_j-y_j+\beta-1}\right]  d\theta \\
&=&  \prod_{j=1}^{10} \left[\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}
\frac{\Gamma(\alpha+y_j)\Gamma(\beta+n_j-y_j)}{\Gamma(\alpha+\beta+n_j)}\right]
(\alpha+\beta)^{-5/2}
\end{eqnarray*}
...
I just set u.grid and v.grid and run the provided code (with the u.grid and v.grid lines commented out). It makes a contour plot of the joint posterior over the range of values provided.
Here's my guesses:
...
The code snippet makes a matrix post, which is 100 $\times$ 100 and contains the normalized probabilities of all the pairs of u and v values. 
I find the rowSums(post) to get the marginal distribution of u, over the u.grid values. 
I sample the index (sampsi) of the u's instead fo the values so that I can take the right row of the post matrix in the next step. 
Once we picked row sampsi[i] of the u's, I can pick out the values from the u.grid vector.
We use the same picked row, sampsi[i], to get the conditional distribution of v's. 
For the v's, I don't need the index for anything, so I just sample from the values.

For each (u,v) pair, I calculate alpha and beta. 
These are used to sample $n$ theta's from the conditional posterior for theta.

You don't need to show a hexbin, but plotting the 5000 (u,v) pairs is not very revealing, so I show that the samples follow the contours from the last part.
I asked for the 12 marginal histograms for alpha, beta, and 10 theta's.
...
The posterior means are similar to the raw proportions, but they are shrunk towards the overall proportion and the shrinking is a bit more for the locations with the largest and smallest raw proportions. 

I rewrote this, but you could have also looked at the bioassay example plot code:
